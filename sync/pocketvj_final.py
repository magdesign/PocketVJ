...
# Button Klasse
class Button(object):
    def __init__(self, gpio_input, start_command = False, stop_command = False, callback = False):
    ...
    def update_state(self):
        ...
        if ((self.input_buffer_prev == 0) and (self.input_buffer == 1)): # button press (entprellt)
            ...
            print "%d: %s" % (self.gpio_input, self.state)
...
class Buttons(object):
    def __init__(self, button_data):
        ...
        for button in button_data:
            self.buttons.append(Button(button[0], button[1], button[2], self.clear_buttons))

        while(True):
            for button in self.buttons:
                button.update_state()
            
    def clear_buttons(self, gpio_input):
        for button in self.buttons:
            if button.gpio_input != gpio_input:
                button.state = False
                if button.stop_command: button.stop_command()
